...
# 2: Considero que los espacios extra que tiene la cadena pueden causar problemas más adelante en el filtrado de datos y puede que afecte nuestro resultado si lo dejamos con los espacios 
# 3: Normalmente las edades son un numero redondeado, de la misma forma si realizamos alguna suma de varias edades puede causar un sesgo en nuestro trabajo.
# 4: Es raro que las listas esten todas en mayúsculas y al momento de buscar igual en la base de datos pueden no aparecer los datos exactos si algunos se encuentran en mayúsculas y otros en minúsculas.
# # Ejercicio 1
# 
...
while total_amount_spent < target_amount: # escribe tu código aquí
	...
	total_amount_spent += 1 # escribe tu código aquí
...
user_info = "User " + user_id + " is " +  user_name[0] + " who is " + str(user_age) + " years old." # escribe tu código aquí
...
for age in users:
    if age[2] < 30:   #escribe tu código aquí
       print(age[1][0])
...
for user in users:
    spendings_list = user[-1] # extrae la lista de gastos de cada usuario o usuaria y suma los valores
    total_spendings = sum(spendings_list) # suma los gastos de todas las categorías para obtener el total de un usuario o una usuaria en particular
    if user[2] < 30 and total_spendings < 1000:   #escribe tu código aquí
       print(user[1][0])
